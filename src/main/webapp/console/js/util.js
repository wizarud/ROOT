	...
	return $.ajax({
		...
		success : function(json, status, xhr) {
			...
			for (var i = 0; i < userBotList.length; i++) {
				...
				var readableBotId = decodeURI(obj);
				if (obj === botId) {
					$(elem).append("<option value=\"" + obj + "\" selected>" + readableBotId + "</option>");
				} else {
					$(elem).append("<option value=\"" + obj + "\">" + readableBotId + "</option>");
				}
			}
			overlayPopup("loader");
		}
		...
		xhr.onload = function() {
			...
			if (xhr.status == 200) {
				...
				if (json.status == "success") {
					...
					console.log("Add " + json.message + " Succeed");
					
					localStorage.botId = json.message;
